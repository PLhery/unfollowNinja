...
function redisTotalWriteCalls(nb: number) {
    const nbCalls = [redis.set, redis.lpush, redis.zadd, redis.zrem, redis.zpush]
        .reduce((p, c) => p + c.mock.calls.length, 0);
    expect(nbCalls).toBe(nb);
}
...
describe('checkFollowers task', () => {
    ...
    test('no follow or unfollow no pagination', async () => {
        ...
        await promisify(cb => task.run(job, cb))();
        expect(queue.save).toHaveBeenCalledTimes(0);
        redisTotalWriteCalls(1);
